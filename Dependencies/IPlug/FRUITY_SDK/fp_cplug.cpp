...
const float Ramp_Res_Unit    = 1.f/1000.f;			// will take this number of samples to go from 0 to 1 at 44100Hz
const float Ramp_Res_Norm    = Ramp_Res_Unit;			// source is -1..1 normalized
const float Ramp_Res_Min     = Ramp_Res_Unit*10.f;	// source is -1..1 normalized, but quicker ramping, with minimum safety
const float Ramp_Res_FromInt = Ramp_Res_Unit*32768.f;	// source is integer
...
// create the object
TCPPFruityPlug::TCPPFruityPlug(intptr_t SetHostTag, TFruityPlugHost *SetPlugHost, HINSTANCE instance, const char* BundleId)
    : TFruityPlug()
{
    HostTag = SetHostTag;
    ...
    GetPluginPath();
#elif __APPLE__
    GetApplePluginPath(PluginPath, BundleId);
#endif
...
void STDMETHODCALLTYPE TCPPFruityPlug::DestroyObject()
{
#ifndef __APPLE__
...
{
    PlugHost->Dispatcher(HostTag, FHD_OnHint_Direct, 0, (size_t)Msg);
}
...
void STDMETHODCALLTYPE TCPPFruityPlug::NewTick()
{
}

intptr_t STDMETHODCALLTYPE TCPPFruityPlug::Dispatcher(intptr_t ID, intptr_t Index, intptr_t Value)
{
    return 0;
}

void STDMETHODCALLTYPE TCPPFruityPlug::Eff_Render(PWAV32FS SourceBuffer, PWAV32FS DestBuffer, int Length)
{
}

void STDMETHODCALLTYPE TCPPFruityPlug::Gen_Render(PWAV32FS DestBuffer, int &Length)
{
}

void STDMETHODCALLTYPE TCPPFruityPlug::GetName(int Section, int Index, int Value, char *Name)
{
}

void STDMETHODCALLTYPE TCPPFruityPlug::Idle_Public()
{
}

int STDMETHODCALLTYPE TCPPFruityPlug::ProcessParam(int Index, int Value, int RECFlags)
{
    return Value;
}

void STDMETHODCALLTYPE TCPPFruityPlug::SaveRestoreState(IStream *Stream, BOOL Save)
{
}

TVoiceHandle STDMETHODCALLTYPE TCPPFruityPlug::TriggerVoice(PVoiceParams VoiceParams, intptr_t SetTag)
{
    return 0;
}

void STDMETHODCALLTYPE TCPPFruityPlug::Voice_Kill(TVoiceHandle Handle)
{
}

int STDMETHODCALLTYPE TCPPFruityPlug::Voice_ProcessEvent(TVoiceHandle Handle, int EventID, int EventValue, int Flags)
{
    return 0;
}

void STDMETHODCALLTYPE TCPPFruityPlug::Voice_Release(TVoiceHandle Handle)
{
}

int STDMETHODCALLTYPE TCPPFruityPlug::Voice_Render(TVoiceHandle Handle, PWAV32FS DestBuffer, int &Length)
{
    return 0;
}

int STDMETHODCALLTYPE TCPPFruityPlug::ProcessEvent(int EventID, int EventValue, int Flags)
{
    return 0;
}

void STDMETHODCALLTYPE TCPPFruityPlug::MIDITick()
{
}

void STDMETHODCALLTYPE TCPPFruityPlug::MIDIIn(int &Msg)
{
}
...
void STDMETHODCALLTYPE TCPPFruityPlug::MsgIn(intptr_t Msg)
{
}
...
{
	int result = int(Min + (Value * 1.0f / (Max-Min+1) * (Max-Min+1)));
	if (result > Max)
		result = Max;
	return result;
}
