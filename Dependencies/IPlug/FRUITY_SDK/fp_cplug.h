...
public:
    ...
    virtual intptr_t STDMETHODCALLTYPE Dispatcher(intptr_t ID, intptr_t Index, intptr_t Value);
    ...
    // events
    virtual int STDMETHODCALLTYPE ProcessEvent(int EventID, int EventValue, int Flags);
    ...
    // voice handling
    virtual TVoiceHandle STDMETHODCALLTYPE TriggerVoice(PVoiceParams VoiceParams, intptr_t SetTag);
    virtual void STDMETHODCALLTYPE Voice_Release(TVoiceHandle Handle);
    virtual void STDMETHODCALLTYPE Voice_Kill(TVoiceHandle Handle);
    ...
    // FPF_EventController
    virtual void STDMETHODCALLTYPE NewTick();

    virtual void STDMETHODCALLTYPE MIDITick();

	virtual void STDMETHODCALLTYPE MIDIIn(int &Msg);

	// buffered messages to itself (see PlugMsg_Delayed)
    virtual void STDMETHODCALLTYPE MsgIn(intptr_t Msg);
	virtual int STDMETHODCALLTYPE OutputVoice_ProcessEvent(TOutVoiceHandle Handle, int EventID, int EventValue, int Flags) { return 0; };
	virtual void STDMETHODCALLTYPE OutputVoice_Kill(TVoiceHandle Handle) {};
    ...
    TCPPFruityPlug(intptr_t SetHostTag, TFruityPlugHost *SetPlugHost, HINSTANCE instance, const char* BundleId); // macOS needs BundleId windows needs HINSTANCE
