...
typedef unsigned long FL_ULONG;
typedef long FL_HRESULT;
typedef unsigned long long FL_UFL_LARGE_INTEGER;
typedef long long FL_LARGE_INTEGER;
#endif
...
virtual FL_ULONG STDMETHODCALLTYPE AddRef( void) = 0;
virtual FL_ULONG STDMETHODCALLTYPE Release( void) = 0;
virtual FL_HRESULT STDMETHODCALLTYPE Read(void *pv, FL_ULONG cb, FL_ULONG *pcbRead) = 0;
virtual FL_HRESULT STDMETHODCALLTYPE Write(const void *pv, FL_ULONG cb, FL_ULONG *pcbWritten) = 0;
virtual FL_HRESULT STDMETHODCALLTYPE Seek(FL_LARGE_INTEGER dlibMove, DWORD dwOrigin, FL_UFL_LARGE_INTEGER *plibNewPosition) = 0;
virtual FL_HRESULT STDMETHODCALLTYPE SetSize(FL_UFL_LARGE_INTEGER libNewSize) = 0;
virtual FL_HRESULT STDMETHODCALLTYPE CopyTo(IStream *pstm, FL_UFL_LARGE_INTEGER cb, FL_UFL_LARGE_INTEGER *pcbRead, FL_UFL_LARGE_INTEGER *pcbWritten) = 0;
virtual FL_HRESULT STDMETHODCALLTYPE Commit(DWORD grfCommitFlags) = 0;
virtual FL_HRESULT STDMETHODCALLTYPE Revert( void) = 0;
virtual FL_HRESULT STDMETHODCALLTYPE LockRegion(FL_UFL_LARGE_INTEGER libOffset, FL_UFL_LARGE_INTEGER cb, DWORD dwLockType) = 0;
virtual FL_HRESULT STDMETHODCALLTYPE UnlockRegion(FL_UFL_LARGE_INTEGER libOffset, FL_UFL_LARGE_INTEGER cb, DWORD dwLockType) = 0;
virtual FL_HRESULT STDMETHODCALLTYPE Stat(void *pstatstg, DWORD grfStatFlag) = 0;
virtual FL_HRESULT STDMETHODCALLTYPE Clone(IStream **ppstm) = 0;
};
#endif
...
public:
    ...
    virtual void STDMETHODCALLTYPE DestroyObject();  // (G)
    virtual intptr_t STDMETHODCALLTYPE Dispatcher(intptr_t ID, intptr_t Index, intptr_t Value) = 0;  // (GM)
    virtual void STDMETHODCALLTYPE Idle_Public() = 0;  // (G) (used to be Idle())
    virtual void STDMETHODCALLTYPE SaveRestoreState(IStream *Stream, BOOL Save) = 0;  // (G)

    // names (see FPN_Param) (Name must be at least 256 chars long)
    virtual void STDMETHODCALLTYPE GetName(int Section, int Index, int Value, char *Name) = 0;  // (GM)

    // events
    virtual int STDMETHODCALLTYPE ProcessEvent(int EventID, int EventValue, int Flags) = 0;  // (GM)
    virtual int STDMETHODCALLTYPE ProcessParam(int Index, int Value, int RECFlags) = 0;  // (GM)

    // effect processing (source & dest can be the same)
    virtual void STDMETHODCALLTYPE Eff_Render(PWAV32FS SourceBuffer, PWAV32FS DestBuffer, int Length) = 0;  // (M)
    // generator processing (can render less than length)
    virtual void STDMETHODCALLTYPE Gen_Render(PWAV32FS DestBuffer, int &Length) = 0;  // (M)

    // voice handling
    virtual TVoiceHandle STDMETHODCALLTYPE TriggerVoice(PVoiceParams VoiceParams, intptr_t SetTag) = 0;  // (GM)
    virtual void STDMETHODCALLTYPE Voice_Release(TVoiceHandle Handle) = 0;  // (GM)
    virtual void STDMETHODCALLTYPE Voice_Kill(TVoiceHandle Handle) = 0;  // (GM)
    virtual int STDMETHODCALLTYPE Voice_ProcessEvent(TVoiceHandle Handle, int EventID, int EventValue, int Flags) = 0;  // (GM)
    virtual int STDMETHODCALLTYPE Voice_Render(TVoiceHandle Handle, PWAV32FS DestBuffer, int &Length) = 0;  // (GM)
    ...
    virtual void STDMETHODCALLTYPE NewTick() = 0;  // (M)

    // (see FHD_WantMIDITick) called when a tick is being played (not mixed) (not used yet)
    virtual void STDMETHODCALLTYPE MIDITick() = 0;  // (S)

    // MIDI input message (see FHD_WantMIDIInput & TMIDIOutMsg) (set Msg to MIDIMsg_Null if it has to be killed)
    virtual void STDMETHODCALLTYPE MIDIIn(int &Msg) = 0;  // (GM)

    // buffered messages to itself (see PlugMsg_Delayed)
    virtual void STDMETHODCALLTYPE MsgIn(intptr_t Msg) = 0;  // (S)

    // voice handling
    virtual int STDMETHODCALLTYPE OutputVoice_ProcessEvent(TOutVoiceHandle Handle, int EventID, int EventValue, int Flags) = 0;  // (GM)
    virtual void STDMETHODCALLTYPE OutputVoice_Kill(TVoiceHandle Handle) = 0;  // (GM)
...
public:
	...
	virtual intptr_t STDMETHODCALLTYPE Dispatcher(TPluginTag Sender, intptr_t ID, intptr_t Index, intptr_t Value) = 0;
    // for the host to store changes
    virtual void STDMETHODCALLTYPE OnParamChanged(TPluginTag Sender, int Index, int Value) = 0;
    // for the host to display hints
    virtual void STDMETHODCALLTYPE OnHint(TPluginTag Sender, char *Text) = 0;

    // compute left & right levels using pan & volume info (OLD, OBSOLETE VERSION, USE ComputeLRVol INSTEAD)
    virtual void STDMETHODCALLTYPE ComputeLRVol_Old(float &LVol, float &RVol, int Pan, float Volume) = 0;

    // voice handling (Sender=voice tag)
    virtual void STDMETHODCALLTYPE Voice_Release(intptr_t Sender) = 0;
    virtual void STDMETHODCALLTYPE Voice_Kill(intptr_t Sender, BOOL KillHandle) = 0;
    virtual int STDMETHODCALLTYPE Voice_ProcessEvent(intptr_t Sender, intptr_t EventID, intptr_t EventValue, intptr_t Flags) = 0;

    // thread synchronisation / safety
    virtual void STDMETHODCALLTYPE LockMix() = 0;  // will prevent any new voice creation & rendering
    virtual void STDMETHODCALLTYPE UnlockMix() = 0;


    // delayed MIDI out message (see TMIDIOutMsg) (will be sent once the MIDI tick has reached the current mixer tick
    virtual void STDMETHODCALLTYPE MIDIOut_Delayed(TPluginTag Sender, intptr_t Msg) = 0;
    // direct MIDI out message
    virtual void STDMETHODCALLTYPE MIDIOut(TPluginTag Sender, intptr_t Msg) = 0;
    ...
    virtual void STDMETHODCALLTYPE AddWave_32FM_32FS_Ramp(void *SourceBuffer, void *DestBuffer, int Length, float LVol, float RVol, float &LastLVol, float &LastRVol) = 0;
    // same, but takes a stereo source
    // note that left & right channels are not mixed (not a true panning), but might be later
    virtual void STDMETHODCALLTYPE AddWave_32FS_32FS_Ramp(void *SourceBuffer, void *DestBuffer, int Length, float LVol, float RVol, float &LastLVol, float &LastRVol) = 0;
    ...
    // see FHLS_ShowDialog
    virtual bool STDMETHODCALLTYPE LoadSample(TSampleHandle &Handle, char *FileName, PWaveFormatExtensible NeededFormat, int Flags) = 0;
    virtual void * STDMETHODCALLTYPE GetSampleData(TSampleHandle Handle, int &Length) = 0;
    virtual void STDMETHODCALLTYPE CloseSample(TSampleHandle Handle) = 0;

    // time info
    // get the current mixing time, in ticks (integer result)
	// obsolete, use FHD_GetMixingTime & FHD_GetPlaybackTime
    virtual int STDMETHODCALLTYPE GetSongMixingTime() = 0;
    // get the current mixing time, in ticks (more accurate, with decimals)
    virtual double STDMETHODCALLTYPE GetSongMixingTime_A() = 0;
    // get the current playing time, in ticks (with decimals)
    virtual double STDMETHODCALLTYPE GetSongPlayingTime() = 0;

    // internal controller
    virtual void STDMETHODCALLTYPE OnControllerChanged(TPluginTag Sender, intptr_t Index, intptr_t Value) = 0;
    ...
    virtual void * STDMETHODCALLTYPE GetSendBuffer(intptr_t Num) = 0;
    ...
    virtual void STDMETHODCALLTYPE PlugMsg_Delayed(TPluginTag Sender, intptr_t Msg) = 0;
    // remove a buffered message, so that it will never be dispatched
    virtual void STDMETHODCALLTYPE PlugMsg_Kill(TPluginTag Sender, intptr_t MSg) = 0;

    // get more details about a sample
    virtual void STDMETHODCALLTYPE GetSampleInfo(TSampleHandle Handle, PSampleInfo Info) = 0;

    // distortion (same as TS404) on a piece of mono or stereo buffer
    // DistType in 0..1, DistThres in 1..10
    virtual void STDMETHODCALLTYPE DistWave_32FM(int DistType, int DistThres, void *SourceBuffer, int Length, float DryVol, float WetVol, float Mul) = 0;
    ...
    virtual void * STDMETHODCALLTYPE GetMixBuffer(int Num) = 0;
    ...
    virtual void * STDMETHODCALLTYPE GetInsBuffer(TPluginTag Sender, int Ofs) = 0;
	...
	virtual bool STDMETHODCALLTYPE PromptEdit(int x, int y, char *SetCaption, char *s, int &c) = 0;

    // same as LockMix/UnlockMix, but stops the sound (to be used before lengthy operations)
    virtual void STDMETHODCALLTYPE SuspendOutput() = 0;
    virtual void STDMETHODCALLTYPE ResumeOutput() = 0;

    // get the region of a sample
    virtual void STDMETHODCALLTYPE GetSampleRegion(TSampleHandle Handle, int RegionNum, PSampleRegion Region) = 0;

    // compute left & right levels using pan & volume info (USE THIS AFTER YOU DEFINED FPF_NewVoiceParams
    virtual void STDMETHODCALLTYPE ComputeLRVol(float &LVol, float &RVol, float Pan, float Volume) = 0;
	...
	virtual void STDMETHODCALLTYPE LockPlugin(TPluginTag Sender) = 0;
	virtual void STDMETHODCALLTYPE UnlockPlugin(TPluginTag Sender) = 0;

    // multithread processing synchronisation / safety
    virtual void STDMETHODCALLTYPE LockMix_Shared_Old() = 0;
    virtual void STDMETHODCALLTYPE UnlockMix_Shared_Old() = 0;
	...
	virtual void STDMETHODCALLTYPE GetInBuffer(TPluginTag Sender, intptr_t Index, PIOBuffer IBuffer) = 0;	// returns (read-only) input buffer Index (or Nil if not available).
	virtual void STDMETHODCALLTYPE GetOutBuffer(TPluginTag Sender, intptr_t Index, PIOBuffer OBuffer) = 0;	// returns (add-only) output buffer Index (or Nil if not available). Use LockMix_Shared when adding to this buffer.


    // output voices (VFX "voice effects")
    virtual TOutVoiceHandle STDMETHODCALLTYPE TriggerOutputVoice(TVoiceParams *VoiceParams, intptr_t SetIndex, intptr_t SetTag) = 0;  // (GM)
    virtual void STDMETHODCALLTYPE OutputVoice_Release(TOutVoiceHandle Handle) = 0;  // (GM)
    virtual void STDMETHODCALLTYPE OutputVoice_Kill(TOutVoiceHandle Handle) = 0;  // (GM)
    virtual int STDMETHODCALLTYPE OutputVoice_ProcessEvent(TOutVoiceHandle Handle, intptr_t EventID, intptr_t EventValue, intptr_t Flags) = 0;  // (GM)
